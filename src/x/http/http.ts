...
import { Http, Request, RequestMethod, RequestOptionsArgs, Response, URLSearchParams, Headers } from '@angular/http';
...
export class HttpApi<T> {
    ...
    Get<T>(subUri: string, q?: any): Observable<T> {
        ...
        request.headers.set('Authorization', getSession('session_id'))
    ...
    Put<T>(sub: string, q?: any, body?: any) {
        const request = this.makeRequest(sub, RequestMethod.Put, q, body)
        request.headers.set('Authorization', getSession('session_id'))
        return this.send(request)
    }
